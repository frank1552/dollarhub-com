...
2. Add these Production environment variables in **Project Settings → Environment Variables**:
...
The OAuth token exchange is handled by the Next.js route at `/api/oauth/token`, so the browser does not call Deriv's token endpoint directly.
